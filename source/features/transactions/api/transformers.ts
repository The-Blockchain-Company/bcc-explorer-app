...
export const transactionDetailsTransformer = (
  ...
  burn:
    tx.mint
      ...
      .sort(sortTokensDesc) || [],
  deposit: Currency.Util.entropicsToBcc(tx.deposit),
  fee: Currency.Util.entropicsToBcc(tx.fee),
  id: tx.hash,
  ...
  inputs: tx.inputs.map((i) => ({
    ...
    tokens: i.tokens
      ...
      .sort(sortTokensDesc),
    value: Currency.Util.entropicsToBcc(i.value),
  })),
  ...
  totalOutput: Currency.Util.entropicsToBcc(tx.totalOutput),
  withdrawals:
    tx.withdrawals?.filter(isDefined).map((i) => ({
      address: i.address,
      value: Currency.Util.entropicsToBcc(i.amount),
    })) || [],
});
